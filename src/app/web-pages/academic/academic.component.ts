...
export class AcademicComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.isGetData) {
      this.getData(`${this.url}?facet=on&q=*:*&start=${this.start}&rows=${this.rows}&fq=type:AcademicSearch`);
    }
  ...
  call () {
    ...
    if (urlArr.length > 0) {
      this.getData(`${this.url}?facet=on&q=${content}&start=${this.start}&rows=${this.rows}&fq=type:AcademicSearch&${urlArr.join('&')}`)
    } else {
      this.getData(`${this.url}?facet=on&q=${content}&start=${this.start}&rows=${this.rows}&fq=type:AcademicSearch`)
    }
  }
